...
{
    /// <summary>
    /// Defines a <c>TagHelper</c> that allows the attached element to be suppressed if it does not meet the authorization requirements.
    /// </summary>
    ...
    public class AuthorizeTagHelper : TagHelper, IAuthorizeData
    {
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            if (!authorizeResult.Succeeded)
                // If so, then we suppress the rendering of the element and its children
